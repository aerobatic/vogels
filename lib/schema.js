...
var Schema = (module.exports = function(config) {
  ...
  var self = this;
  Joi.validate(config, internals.configSchema, { context: context }, function(
    err,
    data
  ) {
    if (err) {
    ...
    if (data.indexes) {
      self.globalIndexes = _.chain(data.indexes)
        .filter({ type: "global" })
        .keyBy("name")
        .value();
      self.secondaryIndexes = _.chain(data.indexes)
        .filter({ type: "local" })
        .keyBy("name")
        .value();
    }
    ...
    if (data.schema) {
      self._modelSchema = _.isPlainObject(data.schema)
        ? Joi.object().keys(data.schema)
        : data.schema;
    } else {
    ...
    self._modelDatatypes = internals.parseDynamoTypes(
      self._modelSchema.describe()
    );
  });
});
...
Schema.types.binarySet = function() {
  var set = Joi.array()
    .includes(Joi.binary(), Joi.string())
    .meta({ dynamoType: "BS" });
  return set;
};
