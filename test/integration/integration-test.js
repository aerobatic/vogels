      ...
      function(callback) {
        async.times(
          ...
          function(n, next) {
            ...
            var p = {
              UserId: userId,
              content: "I love tweeting, in fact Ive tweeted " + n + " times",
              num: n
            };
            if (n % 3 === 0) {
              p.tag = "#test";
      ...
      function(callback) {
        async.times(
          ...
          function(n, next) {
            var director = {
              firstName: "Steven",
              lastName: "Spielberg the " + n,
              titles: ["Producer", "Writer", "Director"]
            };
            var actors = [
              {
                firstName: "Tom",
                lastName: "Hanks",
                titles: ["Producer", "Actor", "Soundtrack"]
              }
            ];
...
describe("Vogels Integration Tests", function() {
  ...
  before(function(done) {
    ...
    async.series(
      [
        ...
        function(callback) {
          User.create(
            { id: "9999", email: "9999@test.com", age: 99, name: "Nancy Nine" },
            callback
          );
        },
  ...
  describe("#update", function() {
    ...
    it("should update age using expected value", function(done) {
      User.update({ id: "9999", age: 100 }, { expected: { age: 99 } }, function(
        err,
        acc
      ) {
        expect(err).to.not.exist;
    ...
    it("should update email using expected that an email already exists", function(done) {
      User.update(
        { id: "9999", email: "new9999@test.com" },
  ...
  describe("#getItems", function() {
    it("should return 3 items", function(done) {
      User.getItems(["userid-1", "userid-2", "userid-3"], function(
        err,
        accounts
      ) {
        expect(err).to.not.exist;
  ...
  describe("#query", function() {
    ...
    it("should return tweets that match expression filters", function(done) {
      Tweet.query("userid-1")
        .filterExpression(
          "#num BETWEEN :low AND :high AND attribute_exists(#tag)"
        )
  ...
  describe("#scan", function() {
    ...
    it("should return users that match expression filters", function(done) {
      User.scan()
        .filterExpression(
          "#age BETWEEN :low AND :high AND begins_with(#email, :e)"
        )
  ...
  describe("timestamps", function() {
    ...
    before(function(done) {
      ...
      ModelCustomTimestamps = vogels.define(
        "vogels-int-test-timestamp-custom",
        {
          hashKey: "id",
          timestamps: true,
          createdAt: "created",
  ...
  describe("#destroy", function() {
    ...
    beforeEach(function(done) {
      User.create(
        { email: "destroy@test.com", age: 20, roles: ["tester"] },
        function(err, acc) {
          expect(err).to.not.exist;
          userId = acc.get("id");
    ...
    it("should destroy item and return old values", function(done) {
      User.destroy({ id: userId }, { ReturnValues: "ALL_OLD" }, function(
        err,
        acc
      ) {
        expect(err).to.not.exist;
  ...
  describe("model methods", function() {
    ...
    it("#update with callback", function(done) {
      Tweet.create(
        { UserId: "tester-2", content: "update test tweet" },
        function(err, tweet) {
          expect(err).to.not.exist;
    ...
    it("#toJSON", function(done) {
      Tweet.create(
        ...
        function(err, tweet) {
          ...
          expect(tweet.toJSON()).to.have.keys([
            "UserId",
            "content",
            "TweetID",
            "PublishedDateTime"
